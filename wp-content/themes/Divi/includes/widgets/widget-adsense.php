<?php class AdsenseWidget extends WP_Widget
{
	...
	function widget( $args, $instance ){
		extract($args);
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Adsense', 'Divi' ) : esc_html( $instance['title'] ) );
	...
	function form( $instance ){
		//Defaults
		$instance = wp_parse_args( (array) $instance, array( 'title' => __( 'Adsense', 'Divi' ), 'adsenseCode'=>'' ) );
		...
		# Adsense Code
		echo '<p><label for="' . $this->get_field_id('adsenseCode') . '">' . __( 'Adsense Code', 'Divi' ) . ':' . '</label><textarea cols="20" rows="12" class="widefat" id="' . $this->get_field_id('adsenseCode') . '" name="' . $this->get_field_name('adsenseCode') . '" >'. $adsenseCode .'</textarea></p>';
	}
